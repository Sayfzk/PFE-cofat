...
\begin{itemize}
    ...
    \item \textbf{Portabilité et déploiement continu :} L'application doit pouvoir être installée sur n'importe quel serveur Windows ou Linux du département IT d'ABM Informatique sans conflit de versions (node modules). La solution est donc pensée pour un déploiement conteneurisé sous Docker, orchestré via un pipeline CI/CD automatisé sous Jenkins, assurant des mises en production fluides et sans régression.
\end{itemize}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.9\textwidth]{img/CasUtilisationGlobal.png}
...
\begin{enumerate}
    \item \textbf{User} : L'utilisateur du système (id, name, email, password haché, role).
    \item \textbf{Site} : Les usines de COFAT (id, code, name, country).
    \item \textbf{Equipment} : Le catalogue local (id, name, type, siteId).
    \item \textbf{EquipmentPlanning} : Les prévisions de machines (machineNeed, availableMachine, load, period).
    \item \textbf{Spaces} : La modélisation des surfaces d'usine (zone, totalArea, occupiedArea, rate).
    \item \textbf{HR} : Les ressources humaines (category, project, effectif_MO_Q0).
    \item \textbf{NonIndustrialBudget} : Les demandes budgétaires (department, area, equipment, qty, currency, unitPrice, totalPrice calculé, status).
    \item \textbf{BudgetNotification} : Le routage des alertes budget (type, message, isRead, fromUser, toRole).
    \item \textbf{Notification} : Les alertes générales du système.
    \item \textbf{StandardInvestment} : Le référentiel des prix (CODE EQ, OPERATIONS, FOURNISSEUR, COUT EURO/BRL/MEX).
\end{enumerate}

\vspace{0.5cm}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/ClassGlobal.png}
